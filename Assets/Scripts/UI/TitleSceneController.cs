...
public class TitleSceneController : MonoBehaviour
{
    ...
    [Header("오브젝트")]
    public SpriteRenderer WomanRenderer;
    ...
    private void OnGameStartPressed()
    {
        ...
        if (OtherTextsGroup != null)
            seq.Join(OtherTextsGroup.DOFade(0f, AnimationDuration * 0.5f));

        // 3. 여인 크기 작아짐 (기존 스케일 기준 곱연산)
        if (WomanRenderer != null)
            seq.Join(WomanRenderer.transform.DOScale(WomanRenderer.transform.localScale * WomanScaleMultiplier, AnimationDuration));

        // 4. 불과 농기구 페이드인 및 여인에 맞춰 작아짐
        ...
        {
            seq.Join(ToolRenderer.DOFade(1f, AnimationDuration));
            seq.Join(ToolRenderer.transform.DOScale(ToolRenderer.transform.localScale * WomanScaleMultiplier, AnimationDuration));
        }

        // 5. 반대로 배경의 달은 점점 커짐
        if (MoonRenderer != null)
            seq.Join(MoonRenderer.transform.DOScale(MoonRenderer.transform.localScale * MoonScaleMultiplier, AnimationDuration));
